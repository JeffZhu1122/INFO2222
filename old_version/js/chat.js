...
Chat.prototype.setup = function() {
  var self = this
  jQuery.ajax({
    url: 'chat/api/info', dataType: 'json',
  ...
  this.form.submit(function(e){
    ...
    jQuery.ajax({
      url: '/chat/api/send_message', dataType: 'json', type: 'POST', data: data,
      success: function(data){
        if(data.error) {
          self.on_error(data)
        } else {
...
Chat.prototype.tick = function() {
  var self = this
  jQuery.ajax({url: 'chat/api/fetch', dataType: 'json', data: {since:this.last_message},
    success: function(data) {
