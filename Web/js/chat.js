...
Chat.prototype.setup = function() {
  var self = this
  jQuery.ajax({
    url: 'posts/api/info', dataType: 'json',
  ...
  this.form.submit(function(e){
    ...
    e.preventDefault();
    if(!self.form.find('textarea[name=text]').val()) return
    if(!self.form.find('input[name=title]').val()) return
    // if(!self.form.find('input[name=nick]').val()) return
    jQuery.ajax({
      url: '/posts/api/send_message', dataType: 'json', type: 'POST', data: data,
      success: function(data){
        if(data.error) {
          self.on_error(data)
        } else {
          self.form.find('textarea[name=text]').focus().val('')
          self.form.find('input[name=title]').focus().val('')
        }
      }
    })
  })
...
Chat.prototype.tick = function() {
  var self = this
  jQuery.ajax({url: 'posts/api/fetch', dataType: 'json', data: {since:this.last_message},
    success: function(data) {
...
Chat.prototype.append_message = function(msg) {
  ...
  node.find('.title').text(msg.title)
  node.find('.comm').text("0 comment")
  this.append_node(node)
}
